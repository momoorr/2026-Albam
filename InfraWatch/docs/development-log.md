## 개발 기록

### 2026-09-03

#### 1. 서버 상태 모니터링 기능 구현

무인매장 인프라 장애를 감지하기 위한 기본 모니터링 기능을 구현했다.

먼저 무인매장의 주요 인프라를 가정하여 POS 서버, DB 서버, 재고 서버를 등록했다.

```python
servers = {
    "POS 서버": "127.0.0.1",
    "DB 서버": "127.0.0.1",
    "재고 서버": "192.0.2.1"
}
```

각 서버의 IP 주소를 대상으로 Ping을 보내 서버의 네트워크 연결 상태를 확인하도록 구현했다.

#### 2. Ping을 이용한 서버 상태 확인
Python의 subprocess 모듈을 사용하여 Windows 환경에서 Ping을 실행했다.

```python
def check_server(ip):
    result = subprocess.run(
        ["ping", "-n", "1", "-w", "1000", ip],
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL
    )

    return result.returncode == 0
```

Ping 명령의 반환값을 이용하여 서버 상태를 판단한다.
* True → 서버 정상
* False → 서버 장애

#### 3. 여러 서버 상태 확인
등록된 서버들을 반복문으로 순회하면서 각각의 상태를 출력하도록 구현했다.

```python
for name, ip in servers.items():
    current_status = check_server(ip)
```

실행 결과:
```
POS 서버 (127.0.0.1) → 🟢 정상
DB 서버 (127.0.0.1) → 🟢 정상
재고 서버 (192.0.2.1) → 🔴 장애
```

192.0.2.1은 테스트를 위해 사용한 도달할 수 없는 IP 주소이다.

#### 4. 주기적인 상태 모니터링
while True와 time.sleep(5)를 사용하여 서버 상태를 5초마다 반복적으로 확인하도록 구현했다.

```python
while True:
    ...
    time.sleep(5)
```

이를 통해 한 번만 상태를 확인하는 것이 아니라 지속적으로 인프라 상태를 모니터링할 수 있도록 했다.

#### 5. 장애 발생 및 복구 상태 감지
서버의 이전 상태를 저장하기 위해 previous_status 딕셔너리를 사용했다.

```python
previous_status = {}
```

현재 상태와 이전 상태를 비교하여 상태가 변경되는 순간을 감지하도록 구현했다.

정상 → 장애

```python
elif previous_status[name] and not current_status:
    print(f"🚨 [장애 발생] {name}에 장애가 발생했습니다.")
```

장애 → 정상

```python
elif not previous_status[name] and current_status:
    print(f"✅ [복구 완료] {name}이 정상적으로 복구되었습니다.")
```

이를 통해 단순히 현재 상태를 보여주는 것뿐만 아니라 장애가 발생한 시점과 복구된 시점의 상태 변화를 감지할 수 있도록 했다.


#### 현재 구현 기능

현재까지 다음 기능을 구현했다.

* 서버 IP 관리
* Ping을 이용한 서버 상태 확인
* 여러 서버 모니터링
* 5초 주기 상태 확인
* 장애 발생 감지
* 장애 복구 감지


---

### 2026-09-04

#### 1. Ping을 이용한 서버 상태 확인

Python의 subprocess를 이용하여 서버의 IP 주소에 Ping을 보내
서버의 네트워크 연결 상태를 확인하도록 구현했다.

```python
def check_server(ip):
    result = subprocess.run(
        ["ping", "-n", "1", "-w", "1000", ip],
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL
    )

    return result.returncode == 0
```
Ping 응답이 있으면 정상, 응답이 없으면 장애로 판단한다.

#### 2. 여러 서버 모니터링

무인매장의 주요 인프라를 가정하여 다음 서버를 구성했다.

* POS 서버
* DB 서버
* 재고 서버

각 서버의 IP 주소를 관리하고 반복적으로 상태를 확인하도록 구현했다.

#### 3. 주기적인 모니터링
while True와 time.sleep(5)를 이용하여
5초마다 서버 상태를 확인하도록 변경했다.

#### 4. 장애 발생 및 복구 감지
이전 상태를 previous_status에 저장하여
정상 → 장애 또는 장애 → 정상으로 상태가 변경되는 순간을 감지하도록 구현했다.

```python
if previous_status[name] and not current_status:
    print("장애 발생")

elif not previous_status[name] and current_status:
    print("복구 완료")
```

#### 5. 시스템 자원 모니터링
psutil 라이브러리를 이용하여 현재 시스템의

* CPU
* Memory
* Disk

사용률을 확인하도록 추가했다.

#### 6. 서비스 포트 상태 확인
Ping만으로는 서버가 살아있는지 여부만 확인할 수 있기 때문에
실제 서비스가 정상적으로 동작하는지 확인하기 위해 TCP 포트 검사를 추가했다.

```python
check_port(ip, port)
```

예를 들어 POS 서버의 경우:
* 서버 Ping 정상
* 8000번 포트 정상

이어야 POS 서비스가 정상이라고 판단할 수 있다.

#### 테스트 결과
Python HTTP 서버를 8000번 포트에서 실행한 결과:
```
POS 서버 (127.0.0.1) → 🟢 정상
포트 8000 → 정상
```

HTTP 서버를 종료하면:
```
POS 서버 (127.0.0.1) → 🟢 정상
포트 8000 → 장애
```
서버 자체는 살아있지만 서비스가 중단된 상황을 구분할 수 있음을 확인했다.

#### 7. 서비스 자동복구 기능 구현

서비스 포트 장애가 감지되었을 때 Python을 이용하여
해당 서비스를 자동으로 다시 실행하도록 구현했다.

장애 발생:
```text
🚨 [서비스 장애] POS 서버의 포트 8000에 장애가 발생했습니다.
```

자동 복구:
```text
🔧 POS 서비스 자동 복구를 시도합니다.
🔄 POS 서비스 재시작 명령을 실행했습니다.
```

복구 확인:
```text
POS 서버 → 🟢 정상
포트 8000 → 🟢 정상
```

이를 통해 서비스 장애를 감지하고 자동으로 복구하는
기본적인 장애 대응 흐름을 구현했다.

지금까지:

```text
Python
 ├─ 서버 Ping
 ├─ CPU / Memory / Disk
 ├─ 포트 확인
 ├─ 장애 감지
 └─ 자동복구
```

다음: 
```text
Python Monitor
       ↓
Docker Container
       ↓
POS 서비스
       ↓
Container 강제 종료
       ↓
Python이 장애 감지
       ↓
Docker Container 재시작
       ↓
서비스 복구
```


---

### 2026-09-07

#### 1. psutil 디스크 사용량 오류 수정

Windows 환경에서 `psutil.disk_usage()` 실행 시
`SystemError: argument 1 (impossible<bad format char>)` 오류가 발생했다.

디스크 사용량 확인 기능을 Python 기본 라이브러리인 `shutil.disk_usage()`를
사용하는 방식으로 변경하여 해결했다.

- CPU → psutil
- Memory → psutil
- Disk → shutil

#### 2. Docker 기반 POS 서버 구축 및 자동복구 준비

기존 Python 모니터링 시스템에서 감시하던 POS 서비스를 Docker 컨테이너로 구성하고,
향후 장애 발생 시 Python Monitor가 Docker 컨테이너를 자동으로 재시작하도록 구현한다.

#### 작업 내용

#### 1) Docker 환경 확인
- Windows 환경에서 Docker Desktop 설치
- WSL 2 사용 환경 확인
- `wsl --status` 명령으로 WSL 2 정상 동작 확인
- Docker 기본 배포판이 `docker-desktop`으로 설정된 것을 확인

#### 2) POS Docker 이미지 생성
프로젝트 경로:

`C:\Users\nthca\OneDrive\바탕 화면\Albam`

Docker POS 서버 경로:

`Albam/docker/pos`

Dockerfile 작성:

```dockerfile
FROM python:3.12-slim

WORKDIR /app

EXPOSE 8000

CMD ["python", "-m", "http.server", "8000"]
```

### 2026-09-08

#### 3) POS Docker 컨테이너 실행

Docker 이미지를 기반으로 POS 컨테이너를 생성하고 실행하였다.

```powershell
docker run -d --name pos-server -p 8000:8000 infrawatch-pos
```
실행 후 docker ps 명령을 통해 POS 컨테이너가 정상적으로 실행 중인 것을 확인하였다.

```powershell
infrawatch-pos   pos-server   Up
0.0.0.0:8000->8000/tcp
```

#### 4) Python Monitor와 Docker 자동복구 연동

기존 Python HTTP 서버를 직접 실행하는 방식에서
Docker 컨테이너를 재시작하는 방식으로 자동복구 로직을 변경하였다.

```python
subprocess.Popen(
    ["python", "-m", "http.server", "8000"]
)
```

변경 방식:
```python
subprocess.Popen(
    ["python", "-m", "http.server", "8000"]
)
```
이를 통해 POS 서비스 장애 발생 시
Python Monitor가 Docker 컨테이너를 자동으로 재시작하도록 구성하였다.

5) Docker 자동복구 테스트

POS Docker 컨테이너를 의도적으로 중지하여 장애 상황을 재현하였다.

```powershell
docker stop pos-server
```
Python Monitor에서 POS의 8000번 포트 장애를 감지하고
자동복구 명령을 실행하는 것을 확인하였다.
```
🚨 [서비스 장애] POS 서버의 포트 8000에 장애가 발생했습니다.
🔧 POS Docker 서비스 자동 복구를 시도합니다.
🔄 POS Docker 컨테이너 재시작 명령을 실행했습니다.
```

이후 다음 모니터링 주기에서 POS 서비스가 정상적으로 복구된 것을 확인하였다.

```
포트 8000 → 🟢 정상
✅ [서비스 복구] POS 서버의 포트 8000가 정상적으로 복구되었습니다.
```

**테스트 결과**
```text
POS Docker 컨테이너 정상 실행 확인
Python Monitor를 통한 서비스 장애 감지 성공
Docker 컨테이너 자동 재시작 성공
장애 발생 후 서비스 정상 복구 확인
```

이를 통해 Python 기반 인프라 모니터링 시스템과
Docker 기반 서비스 자동복구 기능을 연동하였다.

### 2026-09-10

#### 1. 웹 대시보드 구현

Python Monitor에서 생성되는 인프라 상태 정보를
웹 브라우저에서 확인할 수 있도록 모니터링 대시보드를 구현하였다.

기존 Python Monitor는 터미널에서만 상태를 확인할 수 있었기 때문에
`status.json` 파일에 현재 시스템 및 서버 상태를 저장하고,
웹 페이지에서 해당 JSON 데이터를 읽어 화면에 표시하도록 구성하였다.

#### 2. 상태 정보 JSON 저장

Python Monitor에서 수집한 CPU, Memory, Disk 사용률과
각 서버의 상태 및 서비스 포트 상태를 JSON 형식으로 저장하도록 구현하였다.

```json
{
    "system": {
        "cpu": 11.7,
        "memory": 90.1,
        "disk": 54.3
    },
    "servers": {
        "POS 서버": {
            "ip": "127.0.0.1",
            "port": 8000,
            "server_status": true,
            "port_status": true
        }
    }
}
```

이를 통해 Python Monitor와 웹 대시보드가
JSON 파일을 매개로 상태 정보를 공유할 수 있도록 구성하였다.

#### 3. 웹 대시보드 구성

HTML과 CSS를 이용하여 다음 정보를 표시하는
웹 기반 모니터링 화면을 구현하였다.

* CPU 사용률
* Memory 사용률
* Disk 사용률
* 서버 상태
* 서비스 포트 상태
* 마지막 확인 시간

서버별로 IP 주소와 서버 상태, 서비스 포트 상태를
구분하여 표시하도록 구성하였다.

#### 4. 웹 서버 실행

Python의 http.server를 이용하여 프로젝트 루트 디렉터리를
웹 서버의 기준 경로로 설정하였다.

```json
python -m http.server 8080 --directory "C:\Users\nthca\OneDrive\바탕 화면\Albam"
```

웹 브라우저에서 다음 주소로 접속하여
대시보드가 정상적으로 표시되는 것을 확인하였다.

#### 5. 상태 정보 자동 갱신

웹 페이지에서 status.json을 주기적으로 다시 읽도록 구현하여
Python Monitor가 생성하는 최신 상태 정보를 화면에 반영하도록 구성하였다.

```
loadStatus();

setInterval(loadStatus, 5000);
```

이를 통해 웹 대시보드에서도
5초 주기로 서버 상태가 갱신되는 것을 확인하였다.

**테스트 결과**
```text
Python Monitor 실행
        ↓
status.json 생성 및 갱신
        ↓
웹 대시보드에서 JSON 데이터 조회
        ↓
CPU / Memory / Disk 표시
        ↓
서버 및 서비스 포트 상태 표시
        ↓
5초 주기로 상태 갱신 확인
```

이를 통해 터미널 기반으로 확인하던 인프라 상태를
웹 브라우저에서도 확인할 수 있도록 모니터링 환경을 확장하였다.

#### 6. 장애 이력 저장 기능 구현

장애 발생 및 복구 상황을 기록하기 위해
incidents.json 파일에 장애 이력을 저장하도록 구현하였다.

장애가 발생하거나 복구될 때 시간, 서버명, IP 주소,
서비스 포트, 장애 유형 및 상태를 기록하도록 구성하였다.

```
def save_incident(name, ip, port, failure_type, status):
    incident = {
        "time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "server": name,
        "ip": ip,
        "port": port,
        "failure_type": failure_type,
        "status": status
    }
```

장애 발생 시:
```
장애 발생
```

복구 완료 시:
```
복구 완
```

상태를 각각 기록하도록 구현하였다.

#### 7. 장애 이력 저장 테스트

POS Docker 컨테이너를 의도적으로 중지하여
서비스 장애 상황을 재현하였다.

```powershell
docker stop pos-server
```

Python Monitor가 POS의 8000번 포트 장애를 감지한 후
Docker 컨테이너를 자동으로 재시작하였다.

이 과정에서 장애 및 복구 이력이 incidents.json에 저장되는 것을 확인하였다.

```json
[
    {
        "time": "2026-09-11 14:08:44",
        "server": "POS 서버",
        "ip": "127.0.0.1",
        "port": 8000,
        "failure_type": "서비스 포트 장애",
        "status": "장애 발생"
    },
    {
        "time": "2026-09-11 14:08:53",
        "server": "POS 서버",
        "ip": "127.0.0.1",
        "port": 8000,
        "failure_type": "서비스 포트 장애",
        "status": "복구 완료"
    }
]
```

**테스트 결과**

```text
POS Docker 컨테이너 중지
        ↓
POS 서비스 포트 장애 감지
        ↓
장애 발생 이력 저장
        ↓
Docker 컨테이너 자동 재시작
        ↓
POS 서비스 복구
        ↓
복구 완료 이력 저장
```

#### 8. 웹 대시보드 장애 이력 표시

incidents.json에 저장된 장애 이력을
웹 대시보드에서도 확인할 수 있도록 기능을 추가하였다.

JavaScript의 fetch()를 이용하여 장애 이력 JSON 파일을 읽고
테이블 형태로 화면에 표시하도록 구현하였다.

표시 항목은 다음과 같다.

* 시간
* 서버
* IP
* 포트
* 장애 유형
* 상태

최근 장애 이력이 먼저 표시되도록 구성하였다.

```
const response = await fetch(
    "../incidents.json?time=" + Date.now()
);
```

#### 9. 장애 이력 자동 갱신

서버 상태뿐만 아니라 장애 이력도
5초마다 다시 조회하도록 구성하였다.

```
loadIncidents();

setInterval(loadIncidents, 5000);
```

이를 통해 새로운 장애 또는 복구 이력이 발생하면
웹 대시보드에서도 자동으로 갱신되도록 구현하였다.

#### 10. 서버 상태 요약 기능 추가

웹 대시보드 상단에 전체 서버 수,
정상 서버 수, 장애 서버 수를 표시하도록 추가하였다.

서버의 Ping 상태와 서비스 포트 상태가
모두 정상인 경우 정상 서버로 판단하도록 구성하였다.

```
전체 서버 → 3
정상 서버 → 1
장애 서버 → 2
```

이를 통해 여러 서버의 상태를 개별적으로 확인하는 것뿐만 아니라
전체 인프라 상태를 한눈에 파악할 수 있도록 개선하였다.

#### 11. 장애 이력 상태 시각화

장애 이력 테이블에서 상태에 따라 색상을 구분하도록 구성하였다.

```
장애 발생 → 빨간색
복구 완료 → 초록색
```

이를 통해 장애 이력의 상태를
웹 화면에서 빠르게 구분할 수 있도록 개선하였다.

**현재 구현 기능**

현재까지 다음 기능을 구현하였다.

* 서버 IP 관리
* Ping을 이용한 서버 상태 확인
* 여러 서버 모니터링
* CPU / Memory / Disk 모니터링
* 5초 주기 상태 확인
* 서비스 TCP 포트 확인
* 서버 장애 감지
* 서비스 장애 감지
* 장애 복구 감지
* Docker 기반 POS 서비스 구성
* Docker 컨테이너 자동복구
* 상태 정보 JSON 저장
* 장애 및 복구 이력 JSON 저장
* 웹 기반 모니터링 대시보드
* 서버 상태 요약
* 장애 이력 웹 표시
* 장애 이력 자동 갱신

**현재 시스템 구성**

```
                    웹 브라우저
                         │
                         ▼
                ┌─────────────────┐
                │ Web Dashboard   │
                │ HTML / CSS / JS │
                └────────┬────────┘
                         │
                    JSON 조회
                         │
              ┌──────────┴──────────┐
              ▼                     ▼
        status.json          incidents.json
              ▲                     ▲
              │                     │
              └──────────┬──────────┘
                         │
                         │
                ┌────────▼────────┐
                │ Python Monitor  │
                └────────┬────────┘
                         │
              ┌──────────┼──────────┐
              ▼          ▼          ▼
            POS 서버    DB 서버    재고 서버
              │
              ▼
        Docker Container
              │
              ▼
         POS 서비스
```
다음 구현:

```
장애 시나리오 정리
        ↓
POS 서비스 장애 테스트
        ↓
장애 감지
        ↓
Docker 자동복구
        ↓
복구 상태 확인
        ↓
장애 이력 확인
        ↓
발표용 시연 흐름 구성
```

